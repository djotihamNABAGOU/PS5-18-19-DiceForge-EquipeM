/*
     ...
     et indique de payer en gros des faces de Moon dans l'orde 1S 1S  
    ...
    public SanctuarysFaces choiceMoonFace(ArrayList<SanctuarysFaces> list){
        ...
        for(int a=0;a<list.size();a++){
                ...
                switch(list.get(a).getOffered().get(0).getValue()){
                    case 1 : moonFaceOneAvailable = true;
                             break;
                    case 2 : moonFaceTwoAvailable = true;
                             break;
                }
        ...
        if((val==1) && moonFaceOneAvailable){
            val = 1;
            return list.get(indice);
        }else if(moonFaceTwoAvailable){
            for(int a=0;a<list.size();a++){
