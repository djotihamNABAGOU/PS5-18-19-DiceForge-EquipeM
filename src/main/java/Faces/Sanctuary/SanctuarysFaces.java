...
public class SanctuarysFaces extends GeneralFace {
    ...
    public int makeEffectFaceMultiplier(int action, int favMin, Temple temple, int numBot,
        ...
        if (this.mode.equals("Add")) {
            ...
            int choice = bot.getStrategy().giveMeYourChoice(offeredGeneralFaces, 0);
    ...
    public int giveMeShieldGain(int action, Bot bot, int numBot, ShieldOfTheGuardianFace face, ArrayList<GeneralFace>[] data, Bot... listBot) {
        ...
        if (this.mode.equals("Choice")) {
            ArrayList<GeneralFace> offeredGeneralFaces = new ArrayList<>();
            for (SimpleFace faceG: Offered) {
                offeredGeneralFaces.add(faceG);
            }
            int number = bot.getStrategy().giveMeYourChoice(offeredGeneralFaces, 0);
    ...
    public ArrayList<String> getProperties() {
        ...
        for (SimpleFace face : Offered) {
            ...
            switch (face.getType()) {
                case "G":
                    gold = Integer.valueOf(simpleFaceList.get(0));
                    break;
                case "S":
                    sun = Integer.valueOf(simpleFaceList.get(1));
                    break;
                case "M":
                    moon = Integer.valueOf(simpleFaceList.get(2));
                    break;
                case "Gl":
                    glory = Integer.valueOf(simpleFaceList.get(3));
                    break;
            }
        }
